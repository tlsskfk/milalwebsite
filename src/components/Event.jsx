...
const Event = () => {
  ...
  return (
      ...
      <div className="flex justify-between w-full absolute top-[50%] h-8 md:h-16 md:px-4">
        ...
        <img src={arrow_right} className='opacity-20 hover:opacity-75 cursor-pointer hover:scale-125' onClick={() => {`${event === slideShow.length-1 ? setEvent(0) : setEvent((prev)=> prev+1)}`}}/>
      </div>
      <div className="flex justify-end w-full absolute bottom-1 md:bottom-4 pr-8">
      ...
      {slideShow.map((slide, index) => (
        <MagicRainbowBg key={slide.label} alt={slide.label} className={`${event === index ? 'px-4 py-20 rounded-xl md:py-32 md:px-20 md:rounded-3xl ': 'p-0' } z-[1] shadow-inner`}>
